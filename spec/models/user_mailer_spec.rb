...
describe UserMailer do
  ...
  describe '#project_invitation' do
    ...
    it 'should have link to create account with email in params' do
      @email.should have_body_text(/#{new_user_url(:host => 'localhost:3000', :email => 'yahoo@yahoo.org').gsub('?', '\?')}/)
    end
  end
  ...
  describe '#project_removal' do
    ...
    it 'should contain the address of the remover' do
      @email.should have_body_text(/remover@toto.com/)
    end
  end
  ...
  describe '#error_notify' do
    ...
    it 'should have link to error in body' do
      @email.should have_body_text(/#{project_error_url(@project, @error, :host => 'localhost:3000').gsub('?', '\?')}/)
    end
  end
  ...
  describe '#error_digest_notify' do
    ...
    it 'should have link to error in body' do
      @email.should have_body_text(/#{project_error_url(@project, @errors.first, :host => 'localhost:3000').gsub('?', '\?')}/)
    end
